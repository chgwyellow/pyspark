...
#### Automated Deployment (Using deploy.sh - Production Ready)
...
- Automatically checks if Docker container is running
- Starts the container if needed
- Executes the production pipeline (`maintenance_daily_ingest`)
- Validates execution status and provides clear feedback
- Uses default configurations (13 partitions, 24 salt factor)

**Customization**: Edit `deploy.sh` to modify `DEFAULT_PARTITIONS` and `DEFAULT_SALT` variables for your needs.
...
- [Data Persistence Strategy](docs/06_pipeline/6_3_data_persistence_strategy.md) - Storage optimization and lifecycle management
...
├── docs/                       # Learning documentation (29 files)
│   ├── 01_fundamentals/
│   │   ├── 1_1_spark_core.md
...
│   ├── jobs/                   # Learning scripts (Chapters 1-5, 17 files)
│   │   ├── 01_fundamentals/    # 2 scripts
│   │   │   ├── 1_1_spark_core.py
│   │   │   └── 1_2_data_ingestion.py
│   │   ├── 02_dataframe_operations/  # 3 scripts
│   │   │   ├── 2_1_basic_operations.py
│   │   │   ├── 2_2_casting_and_nulls.py
│   │   │   └── 2_3_complex_types.py
│   │   ├── 03_advanced_operations/   # 3 scripts
│   │   │   ├── 3_1_aggregation.py
│   │   │   ├── 3_2_multi_table_join.py
│   │   │   └── 3_3_storage_formats.py
│   │   ├── 04_data_integration/      # 6 scripts
...
```bash
# First time setup: Make deploy.sh executable
chmod +x deploy.sh
...
# 2. Start container if needed (with 5s initialization wait)
# 3. Execute maintenance_daily_ingest pipeline
# 4. Report success/failure status
```

**Customize deployment parameters** by editing `deploy.sh`:

```bash
DEFAULT_PARTITIONS="13"  # Adjust Spark shuffle partitions
DEFAULT_SALT="24"        # Adjust salting factor for skew optimization
```
